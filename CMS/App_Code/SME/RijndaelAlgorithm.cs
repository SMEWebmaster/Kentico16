...
public class RijndaelAlgorithm
{
    ...
    {
        return Encrypt((GetTimeStamp().Trim() + "|" + returnUrl + "|" + user + "|" + password + "|" + rememberMe), vendorPassword, vendorBlock);
    }
    ...
    private static string Encrypt(string text, string password, string block)
    {
        ...
        try
        {
            using (SymmetricAlgorithm provider = new AesManaged())
            {
                using (MemoryStream buffer = new MemoryStream())
                {
                    using (CryptoStream writer = new CryptoStream(buffer, provider.CreateEncryptor(FromHexaDecimal(password), FromHexaDecimal(block)), CryptoStreamMode.Write))
                    {
                        ASCIIEncoding encoding = new ASCIIEncoding();
                        byte[] bytes = encoding.GetBytes(text);

                        SessionHelper.SetValue("OriginalBytes", ToHexaDecimal(bytes));
                        SessionHelper.SetValue("VendorPW", password);
                        SessionHelper.SetValue("VendorBlock", block);
                        writer.Write(bytes, 0, bytes.Length);

                        writer.FlushFinalBlock();
                    }
                    
                    encryptedString = ToHexaDecimal(buffer.ToArray());

                    SessionHelper.SetValue("OriginalVendorToken", encryptedString);
                    SessionHelper.SetValue("VendorTokenText", text);
                }
            }
        }
        catch (Exception excp)
        {
            EventLogProvider.LogException("VendorToken", "Encrypt", excp, SiteContext.CurrentSiteID, "Text: " + text);
        }
    ...
    private static string GetTimeStamp()
    {
        ...
        SessionHelper.SetValue("BufferDateTime", now);
        ...
        bufferString = digitsOnly.Replace(bufferString, "");

        SessionHelper.SetValue("BufferDateTimeString", bufferString);
