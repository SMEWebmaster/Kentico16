...
function SelectAllItems(checkbox, hash) {
    ...
    string ICallbackEventHandler.GetCallbackResult()
    {
        ...
        if (!string.IsNullOrEmpty(callbackValues))
        {
            ...
            var paramFormat = new Regex(String.Format(@"^(?<items>{0}.*{0}|{0})\|(?<values>({0}.*{0})|([^{0}]*))#(?<hash>.*)$", Regex.Escape(valuesSeparator)));
            ...
            if (paramMatch.Success)
            {
                ...
                if (ValidationHelper.ValidateHash(value, hash, redirect:false))
                {
