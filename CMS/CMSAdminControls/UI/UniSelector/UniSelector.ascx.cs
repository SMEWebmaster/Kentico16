...
public partial class CMSAdminControls_UI_UniSelector_UniSelector : UniSelector, IPostBackEventHandler, ICallbackEventHandler
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if ((mode == SelectionModeEnum.Multiple) && (gridView.HeaderRow != null))
        {
            ...
            chkAll.InputAttributes.Add("onclick", string.Format("US_SelectAllItems('{0}', {1}, this, 'chk{2}')", UniSelectorClientID, ScriptHelper.GetString(ValuesSeparator), mCheckBoxClass));
    ...
    private void LoadObjects()
    {
        if (Object != null)
        {
            ...
            if (DisplayNewButton)
            {
                ...
                {
                    mJavaScript.Append("function US_NewItem_", UniSelectorClientID, "(selectedItem) {{ var url = '", ScriptHelper.ResolveUrl(newUrl), "';selectedItem = US_TrimSeparators(selectedItem, ", ScriptHelper.GetString(ValuesSeparator), @");modalDialog(url.replace(/##ITEMID##/i, selectedItem),'NewItem', ", EditDialogWindowWidth, ", ", EditDialogWindowHeight, "); }} \n");
                }
            }
...
function US_EditItem_", UniSelectorClientID, @"(selectedItem) {
    selectedItem = US_TrimSeparators(selectedItem, ", ScriptHelper.GetString(ValuesSeparator), @");
    if (selectedItem == '') {
            ...
            case "select":
                {
                    ...
                    chkCheckbox.InputAttributes.Add("onclick", string.Format("US_ProcessItem('{0}', {1}, this);", UniSelectorClientID, ScriptHelper.GetString(ValuesSeparator)));
...
return false;", UniSelectorClientID, ScriptHelper.GetString(ValuesSeparator), mCheckBoxClass, ScriptHelper.GetString(itemID).Trim('\''));
