...
public partial class CMSWebParts_SME_ContributionList : CMSUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
		...
		else if ((eventFilters[0] == "" || eventFilters.Length == 0) && QueryHelper.Contains("categoryname"))
        {
		...
		else if (eventFilters[0] != "" && QueryHelper.Contains("categoryname"))
        {
            IsApprovalListVisible();
			if(mShowFilter && eventFilters.Length==1 && eventFilters[0] == "uca_sponsored_event")
			{
				query = "SELECT  * FROM ("+query + " AND CategoryName='" + QueryHelper.GetString("categoryname", "") + "') AS EventsList where EventsList.RowNumber=1 ORDER BY StartDate ASC";
				loadCalendarEvents(query, "events");
			...
			{
				string queryForCustomEvents = "SELECT  * FROM (SELECT ROW_NUMBER() OVER (PARTITION BY EventName ORDER BY CMS_DocumentCategory.CategoryID) AS RowNumber, EventName,EndDate,NodeID,NodeAliasPath,StartDate,datediff(minute,'1990-1-1',StartDate) AS DateTicks,EventDetails,EventCategory,Location,CategoryDisplayName FROM View_SME_Content_Event_Joined INNER JOIN CMS_DocumentCategory ON View_SME_Content_Event_Joined.DocumentID=CMS_DocumentCategory.DocumentID INNER JOIN CMS_Category ON CMS_Category.CategoryID=CMS_DocumentCategory.CategoryID WHERE published=1 AND NodeAliasPath LIKE '/Events-Professional-Development/Events/%' AND ClassName='SME.Event'";
				where = "CategoryName='" + QueryHelper.GetString("categoryname", "") + "'";
				loadCalendarEvents(queryForCustomEvents, "events");
			}           		
        }
		else if (eventFilters[0] != "" && !QueryHelper.Contains("categoryname"))
		{
            ...
            {		
			queryForCustomEvents = queryForCustomEvents + " AND (";
				...
				queryForCustomEvents = queryForCustomEvents + where.Substring(0, where.Length - 3) + " )) AS EventsList where EventsList.RowNumber=1 ORDER BY StartDate DESC";
