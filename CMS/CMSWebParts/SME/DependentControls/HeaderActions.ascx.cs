...
public partial class CMSWebParts_SME_DependentControls_HeaderActions : HeaderActions, IPostBackEventHandler
{
    ...
    {
        // check session for reload issue fixing and redirect.
        var v = SessionHelper.GetValue(SMEConstant.ACTIONEVENTNAME_DOCUMENTAPPROVE);
        if (!Equals(v, null))
        {
            SessionHelper.SetValue(SMEConstant.ACTIONEVENTNAME_DOCUMENTAPPROVE, null);
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (this.DocumentManager.Node != null &&
            (this.DocumentManager.Node.WorkflowStepName == SMEConstant.WORKFLOW_PUBLISHED.ToLower() || this.DocumentManager.Node.WorkflowStepName == SMEConstant.WORKFLOW_ARCHIVED.ToLower()) &&
            this.DocumentManager.Node.GetWorkflow() != null)
        {
            this.DocumentManager.Node.CreateNewVersion();
        }
    ...
    private CMSButtonAction GetControlAction(HeaderAction headerAction)
    {
        ...
        if (Enabled && headerAction.Enabled && !headerAction.Inactive)
        {
            ...
            if (!String.IsNullOrEmpty(headerAction.OnClientClick))
            {
                ...
                if (controlAction.Text.ToLower() == ValidationHelper.GetString(GetString("SubmitText"), "save").ToLower() || controlAction.Text.ToLower() == ValidationHelper.GetString(GetString("ApproveButtonText"), "approve & publish").ToLower() || controlAction.Text.ToLower()== "submit for approval")
                {
                    onClickScript = onClickScript + "if(formValid()===false){return false;}";
                }
