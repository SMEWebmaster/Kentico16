...
public partial class CMSWebParts_SME_EditForm : CMSUserControl
{
    #region "Variables"
    ...
    private void ReloadForm()
    {
        ...
        // Reload edit menu
        menuElem.ShowDelete = false;//AllowDelete && Edit;
    ...
    void DocumentManager_OnAfterAction(object sender, DocumentManagerEventArgs e)
    {
        switch (e.ActionName)
        {
            case ComponentEvents.SAVE:
				...
				TreeNode docNode = tree.SelectSingleNode(Node.NodeID, "en-us", "SME.Event");
				docNode.MoveToNextStep();
				
				/*string query="select EventName,email,StartDate,Location,Contact from View_SME_Content_Event_Joined WHERE NodeAliasPath LIKE '/Events-Professional-Development/Events/%' AND DocumentWorkflowStepID='9191' AND ClassName='SME.Event' AND StartDate>=getdate()";
				
				var queryForSubmittedEvent = new QueryParameters(query, null, CMS.DataEngine.QueryTypeEnum.SQLQuery, false);
				DataSet dsForSubmittedEvent = ConnectionHelper.ExecuteQuery(queryForSubmittedEvent);*/
		
				string eventSubmitterEmail=string.Empty;
				string eventContact=string.Empty;
				string eventDateTime=string.Empty;
				//string eventSubmissionDateTime=string.Empty;
				string eventName=string.Empty;
				
				/*if (!DataHelper.DataSourceIsEmpty(dsForSubmittedEvent))
				{
					// Loop through all documents
					foreach (DataRow documentRow in dsForSubmittedEvent.Tables[0].Rows)
					{
						eventSubmitterEmail = documentRow["email"].ToString();//documentRow.Field<string>("email").ToLower();
						eventContact = documentRow["Contact"].ToString();
						eventDateTime = documentRow["StartDate"].ToString();
						//eventSubmissionDateTime = DateTime.Now.ToString();
						eventName = documentRow["EventName"].ToString();
					}
				}*/
				
				eventSubmitterEmail = docNode.GetValue("email").ToString();
				eventContact = docNode.GetValue("location").ToString();
						eventDateTime = docNode.GetValue("StartDate").ToString();
						eventName = docNode.GetValue("EventName").ToString();
				...
				resolver.SetNamedSourceData("company", eventSubmitterEmail);
				resolver.SetNamedSourceData("contact", eventContact);
				...
				resolver.SetNamedSourceData("eventname", eventName);
				
				SendEmail("EventSubmission",resolver,"bencomo@smenet.org;webmaster@smenet.org");
				
				SendEmail("EventApprovalWaiting",null,eventSubmitterEmail);
    ...
    {
        bool RollbackStatus = RollbackPreviousVersion(e);
        Response.Redirect(Request.RawUrl);
    }
    private bool RollbackPreviousVersion(EventArgs e)
    {
        if (this.DocumentManager.Node != null &&
        this.DocumentManager.Node.WorkflowStepName == SMEConstant.WORKFLOW_STEPNAME_EDIT.ToLower() &&
        this.DocumentManager.Node.GetWorkflow() != null)
        {
            int DocumentNodeID = NodeID;
            Utility.RollbackToPreviousVersion(DocumentNodeID);
        }
