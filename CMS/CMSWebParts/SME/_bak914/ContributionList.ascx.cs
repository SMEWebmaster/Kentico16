...
public partial class CMSWebParts_SME_ContributionList : CMSUserControl
{
    ...
    #region "Methods"
      ...
      string query = "SELECT EventName,EndDate,NodeID,NodeAliasPath,StartDate,datediff(minute,'1990-1-1',StartDate) AS DateTicks,EventDetails,EventCategory,Location,CategoryDisplayName FROM View_SME_Content_Event_Joined INNER JOIN CMS_DocumentCategory ON View_SME_Content_Event_Joined.DocumentID=CMS_DocumentCategory.DocumentID INNER JOIN CMS_Category ON CMS_Category.CategoryID=CMS_DocumentCategory.CategoryID WHERE published=1 AND NodeAliasPath LIKE '/Events-Professional-Development/Events/%' AND ClassName='SME.Event' AND StartDate>=getdate()";
      string queryForApprovalList = "SELECT EventName,EndDate,NodeID,NodeAliasPath,StartDate,datediff(minute,'1990-1-1',StartDate) AS DateTicks,EventDetails,EventCategory,Location,CategoryDisplayName FROM View_SME_Content_Event_Joined INNER JOIN CMS_DocumentCategory ON View_SME_Content_Event_Joined.DocumentID=CMS_DocumentCategory.DocumentID INNER JOIN CMS_Category ON CMS_Category.CategoryID=CMS_DocumentCategory.CategoryID WHERE published=1 AND NodeAliasPath LIKE '/Events-Professional-Development/Events/%' AND DocumentWorkflowStepID='9191' AND ClassName='SME.Event' AND StartDate>=getdate()";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (eventFilters[0] != "" && !QueryHelper.Contains("approvalevents"))
        {
        ...
        else if (eventFilters[0] != "" && QueryHelper.Contains("approvalevents"))
        {
            btnNewDoc.Visible = false;
        ...
        {
            if (QueryHelper.Contains("categoryname") && QueryHelper.Contains("approvalevents"))
            {
                btnUsersToBeApproved.Visible = false;
            ...
            else if (QueryHelper.Contains("approvalevents"))
            {
                btnUsersToBeApproved.Visible = false;
                btnCalendar.Visible = true;
                if (Utility.IsUserInAdminRole())
            ...
            else if (QueryHelper.Contains("categoryname"))
            {
    ...
    private void loadCategoryDropDown()
    {
        ...
        string eventTypeList = "SELECT DISTINCT CategoryName, CategoryDisplayName FROM View_SME_Content_Event_Joined INNER JOIN CMS_DocumentCategory ON View_SME_Content_Event_Joined.DocumentID=CMS_DocumentCategory.DocumentID INNER JOIN CMS_Category ON CMS_Category.CategoryID=CMS_DocumentCategory.CategoryID WHERE published=1 and NodeAliasPath LIKE '/Events-Professional-Development/Events/%' AND ClassName='SME.Event' AND StartDate>=getdate()";
    ...
    protected void ddlEventType_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        else if (ddlEventType.SelectedValue.ToString().ToLower() == "select one" || ddlEventType.SelectedValue.ToString().ToLower() == "view all events")
        {
            URLHelper.Redirect(url.Trim());
        }
        else
